...
# Subversion changes for 0.99eDp4
#    - edited entropy_test function. #entropy = entropy + math.log(float(ntopologies)) 
#    - edited to change some function names to work with dendropy 4
#
...
def read_trees(args, i):
    ...
    try:
        for j in range(1, args.nruns + 1):    # Iterates over 1, ..., nruns
            ...
            pp_trees.read_from_path(filename, 'nexus', tree_offset=args.burnin, rooting='force-unrooted')
    except:
        print '''
...
def calcAndSortTreeDists(args, tree_data):
    ...
    try:
        ...
        topo_freqs = []
        symm_diff = treecompare.symmetric_difference
...
def entropy_test(args, trees, topo_freqs):
    ...
    try:
        ...
        ntaxa = len(trees.taxon_namespace)        
        if args.debug:
            print "# Taxa: %d" % ntaxa
