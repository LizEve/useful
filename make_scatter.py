...
#direct output
outpath=os.path.abspath(infile)
...
plt.text(.9*max(x_val)+.1*min(x_val),.9*max(y_val)+.1*min(y_val),'$R^2 = %0.2f$'% Rsqr, fontsize=15)
...
b=str(round(bb,3))
eqn= 'y = '+m+'x + '+b
plt.text(250,13,eqn, fontsize=15)


outfile="RFTL_nolbls.5.10.png"
